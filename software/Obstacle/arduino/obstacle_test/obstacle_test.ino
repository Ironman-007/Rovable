...
uint8_t cmd[7] = {0x00};
uint8_t ack[7] = {0x03, 0x00, 0xAA, 0x00, 0x00, 0x00, 0xAB};
...
void loop() {
  ...
  while ( network.available() ) {
    ...
    SerialUSB.println(cmd[3]);
    if (header.from_node == 0) {// cmd is from master node
      SerialUSB.println("header.from_node");

//      if (cmd[2] == 0xAA) { // start cmd
        network.update();
        network.write(header, &ack, sizeof(ack)); // Send the ack
        SerialUSB.println("network.write");
        cmd_flag = 1;
//      }
    }   
  }
...
void turnonmotor(){
  analogWrite(M1_PWM, 100); // right
  analogWrite(M2_PWM, 100); // left
