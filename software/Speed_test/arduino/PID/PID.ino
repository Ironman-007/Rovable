...
const uint16_t this_node = 1;   // Address of our node in Octal format ( 04,031, etc)
...
void loop() {
  ...
  while ( network.available() ) {
    ...
    if (header1.from_node == 0) {// cmd is from master node
      ...
      if (cmd[1] == 0x00 && cmd[2] == 0x00) { // start cmd
        network.update();
        cmd_flag = 1;
        cali_flag = 0;
        stop_flag = 0;
...
//      data2send[0] = this_node & 0xFF; // Node_address
      ...
      if (dev_encoder2Counter < 0) {
        overflow_flag = 1;
        dev_encoder2Counter += 255;
      }
      ...
      //===== Sending =====//
      if (stop_flag == 0) network.write(header, &data2send, sizeof(data2send)); // Send the data
    }
...
void turnonmotor(){
  analogWrite(M1_PWM, speed_tts + steer); // right
  analogWrite(M2_PWM, speed_tts - steer); // left
